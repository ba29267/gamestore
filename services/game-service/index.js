...
const resolvers = {
  Query: {
    ...
    searchGames: async (_, {
      ...
      try {
        let query = q === '*' ? '*:*' : `(title:${q} OR description:${q})`;

        const filters = [];
        if (genre) filters.push(`genre:"${genre}"`);
        if (platform) filters.push(`platform:"${platform}"`);
        if (priceMin) filters.push(`price:[${priceMin} TO *]`);
        if (priceMax) filters.push(`price:[* TO ${priceMax}]`);
        if (ratingMin) filters.push(`rating:[${ratingMin} TO *]`);

        if (filters.length > 0) {
          query = `(${query}) AND (${filters.join(' AND ')})`;
        }

        let sortParam = 'score desc';
        ...
        const response = await axios.get(`${solrBase}/select`, {
          params: { q: query, rows: limit, start: offset, sort: sortParam, wt: 'json' },
        });
...
async function initializeSolr() {
  try {
    ...
    try {
      // Try to create collection in cloud mode
      await axios.get(`http://${solrHost}:${solrPort}/api/collections?action=CREATE&name=games&numShards=1&replicationFactor=1`);
      console.log('✓ Games collection created');
    } catch (err) {
      if (err.response?.status === 400 || err.response?.status === 409) {
        console.log('✓ Games collection already exists');
      } else {
        console.warn('Collection creation attempt:', err.message);
        // Try alternate core creation method for standalone mode
        try {
          await axios.post(`http://${solrHost}:${solrPort}/api/cores?action=CREATE&name=games&configSet=_default`);
          console.log('✓ Games core created (standalone mode)');
        } catch (err2) {
          if (err2.response?.status !== 400) {
            console.warn('Core creation also failed:', err2.message);
          }
        }
      }
    }
    ...
    try {
      await axios.post(`${solrBase}/schema`, {
        'add-field': [
      ...
      console.warn('Schema configuration warning:', err.message);
    }
  } catch (err) {
...
app.get('/api/v1/games/search', searchRateLimiter, async (req, res) => {
  try {
    const { q = '*', genre, platform, priceMin, priceMax, ratingMin, sort = 'score desc', limit = 10, offset = 0 } = req.query;

    // Build cache key from query parameters
    const cacheKey = `games:search:${q}:${genre || 'all'}:${platform || 'all'}:${priceMin || '0'}:${priceMax || 'inf'}:${ratingMin || '0'}:${sort}:${limit}:${offset}`;
    ...
    let query = q === '*' ? '*:*' : `(title:*${q}* OR description:*${q}* OR genre:*${q}* OR developer:*${q}*)`;
